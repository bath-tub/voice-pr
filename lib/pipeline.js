...
export async function runBatch({ prRef, transcript }, emit = () => {}) {
  ...
  // --- verify + push the agent's commits ------------------------------------
...
export async function verifyAndPushAgentCommits({
  checkout,
  branch,
  committed,
  emit = () => {},
  ...
  await verifyAgentCommits(checkout, emit, { env, runner });
  emit("pushing", { branch });
  await runner("git", ["push", "origin", branch], { cwd: checkout });
  return true;
}

export async function verifyAgentCommits(
  checkout,
  emit = () => {},
  { env = process.env, runner = run } = {}
) {
  const commands = await verificationCommands(checkout, env);

  for (const command of commands) {
    emit("verifying", { command: command.label });
    try {
      await runner("bash", ["-lc", command.shell], { cwd: checkout });
    } catch (e) {
      emit("verification-failed", { command: command.label });
      throw new Error(
        `verification failed before push (${command.label}): ${e.message}`
      );
    }
  }

  emit("verified", { commands: commands.map((c) => c.label) });
}

export async function verificationCommands(checkout, env = process.env) {
  const configured = env.VOICE_PR_VERIFY_CMD?.trim();
  if (configured) {
    return [{ label: "VOICE_PR_VERIFY_CMD", shell: configured }];
  }

  const commands = [];
  if (await canRead(join(checkout, "build.sh"))) {
    commands.push({ label: "./build.sh", shell: "bash ./build.sh" });
  }
  if (await canRead(join(checkout, "test.sh"))) {
    commands.push({ label: "./test.sh", shell: "bash ./test.sh" });
  }

  if (!commands.length) {
    throw new Error(
      "no verification command found before push; set VOICE_PR_VERIFY_CMD or add build.sh/test.sh to the PR repo"
    );
  }
