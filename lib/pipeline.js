...
import { buildPrompt, buildOrchestratorBody } from "./prompt.js";
import {
  assertOrchestrator,
  ensureProject,
  fileWorkItem,
...
/**
 * Orchestrator backend: turn the voice batch into a pogo work item, nudge the
 * mayor, and track it through the polecat + refinery to a merge on the PR
 * branch. Emits the same-shaped progress events as the direct path.
 */
async function runViaOrchestrator(pr, transcript, emit) {
  ...
  // Post the intent trail from the ADAPTER (host-side): pogod reaps the polecat
  // the instant its merge lands, so any post-merge commenting in the polecat
  // itself never runs. We comment here from the merged commits instead.
  let trailComment = null;
  if (outcome.status === "done") {
    try {
      emit("clarifying", { count: 0 });
