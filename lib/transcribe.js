...
export async function transcribe(audio, ext = "webm") {
  ...
  try {
    ...
    // Run both tools inside the temp dir. whisper-cli calls getcwd() at startup
    // (to locate its ggml backends) and aborts hard if the process's working
    // directory has been deleted — e.g. the bridge was launched from a git
    // worktree that was later removed. Pinning cwd to a dir we just created
    // keeps transcription working regardless of where the bridge runs.
    await run("ffmpeg", ["-y", "-i", inPath, "-ar", "16000", "-ac", "1", "-c:a", "pcm_s16le", wav], { cwd: dir });
    await run(WHISPER, ["-m", MODEL, "-f", wav, "-oj", "-of", outPrefix], { cwd: dir });
